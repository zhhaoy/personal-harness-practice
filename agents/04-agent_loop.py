#!/usr/bin/env python3
"""
Agent Loop - 流式输出 + 规划模式 + 卡死检测 + Subagent 隔离 + 预分析计划 + 待办强制完成
...
# ========== 系统提示 ==========
PARENT_SYSTEM_PROMPT = f"""你是一个智能编码助手，当前工作目录: {WORKDIR}。
你可以使用以下工具：
...
SUBAGENT_SYSTEM_PROMPT = f"""你是一个编码子代理，当前工作目录: {WORKDIR}。
你的任务是完成用户给出的子任务，并返回一个简洁的摘要。
你可以使用 bash, read_file, write_file, edit_file, todo 工具，但不能再次生成子代理。
...
PARENT_TOOLS = BASE_TOOLS_DEF + [TODO_TOOL_DEF, TASK_TOOL_DEF]
TOOLS = PARENT_TOOLS
...
def agent_loop(initial_prompt: str, max_iterations: int = 30) -> AgentResult:
    ...
    # print(f"[分析结果] {analysis_text[:200]}")
...
def main():
    print(
        "\n=== 多模型Agent循环 (流式 + 规划 + 卡死检测 + Subagent + 待办强制完成) ==="
    )
